...
cancelReply = function (ev, th) {
    ...
    countTextareaLength();
    fillPlaceholderOfCommentSpace("Please enter your comments");
...
haveComment = function (commentView, arr) {
    ...
    arr.forEach(function (value) {
        ...
        if (value.author !== 'anonymous') {
            htmlText +=
                '<span style="margin: 0 10px;color: lightgray">reply</span>' +
        ...
        if(value.reply.length > 0) {
            htmlText += '<a id="'+ value.id.substring(0, 6) +'"' +
                        '         style="border: none;">' +
                        '  view reply('+ value.reply.length +')</a></span></li></ul>'
        } else {
            htmlText += '</span></li></ul>';
        }
...
showButton = function (arr, sign) {
    ...
    arr.forEach(function (value) {
        ...
        reply.onclick = function (ev) {
            ...
            var str = "reply ".concat(value.commentator);
        ...
        if (value.reply.length > 0) {
            ...
            checkReply.onclick = function (ev) {
                ...
                if(checkReply.innerText.trim().charAt(0) === '查'){
                    ...
                    checkReply.innerText = "Collapse reply";
                } else {
                    toggleBackReplies(parent);
                    checkReply.innerText = "view reply("+ value.reply.length +")";
                }
            };
        }
...
ifHaveReply = function (parentTag, arr, broEle) {
    ...
    arr.forEach(function (value) {
        ...
        if(value.reply.length > 0) {
            ...
            htmlText += '<a id="'+ value.id.substring(0, 6) +'"' +
                        '         style="border: none;">' +
                        'view reply('+ value.reply.length +')</a></span></li>'
        } else {
            htmlText += '</span></li>';
        }
    });
